...
func (m *LaptopCameraManager) startProcess() error {
	...
	args := []string{
		...
		"!", fmt.Sprintf("image/jpeg,width=%d,height=%d,framerate=15/1", m.settings.Width, m.settings.Height),
		"!", "jpegdec",
		"!", "videoconvert",
		// Essential params:
		// bitrate=600: Low bitrate
		// key-int-max=30: Keyframe every 1s
		"!", "x264enc", "bitrate=600", "speed-preset=ultrafast", "tune=zerolatency", "key-int-max=30", "bframes=0",
		// RTP Packetization:
